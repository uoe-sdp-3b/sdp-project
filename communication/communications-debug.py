...
# Replace this file with communications.py
# If you want to see debung statements about comms
        ...
        while (not sent_successfully):
            ...
            if not self.internal_queue.empty():
                ...
                if response not in ERROR_CODES and len(response) == 3:
                    self.queue.put("RESPONSE:--%s--" % response)
                    sent_successfully = True
            
            if (not sent_successfully):
                # Resend command (because no response was ever got)
                self.queue.put("ATTEMPT:%d" % attempt)
                attempt += 1
                self.to_robot(opcode_string)
    ...
    def write_stream(self):
        while True:
            ...
            if not self.write_queue.empty():
                ...
                self.queue.put("OPCODE: %d" % msg_dict["opcode"])
